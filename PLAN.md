...
| 8 Report levels like Supermetrics (2026-09-20): Google Ads (17 levels + GAQL), Facebook Insights, Microsoft Ads (18 levels, Find accounts, tenant), LinkedIn Analytics (pivots and audiences). TikTok stays in `src/connectors/tiktok_ads.js` with `hidden: true` and is not offered | done | `tests/google-ads-query.test.mjs`, `tests/facebook-ads-insights.test.mjs`, `tests/ad-connectors.test.mjs`; Google Ads, Facebook and Microsoft checked against live accounts, LinkedIn from the documentation only |
