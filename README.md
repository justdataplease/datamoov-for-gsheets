...
- **Protect your spreadsheet:** DataMoov validates the complete report before updating output and stops if someone edited or moved its previous results. Reports support up to 30,000 rows within provider and Google Apps Script™ limits.
...
| ------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| Google Ads          | Daily campaign performance, YouTube video campaigns, 17 report levels (account, campaign, ad group, ad, keyword, search term, negative keywords by campaign and by ad group, conversion action, geography, age, gender, audience, landing page, placement, Performance Max asset group, Shopping product), Custom query (GAQL) | Spend, impressions, clicks, conversions, conversion value, optional segments; video views, view rate, CPV and quartiles; every level offers its own dimensions plus **Load columns** for all compatible fields; any other resource through one GAQL query |
| Facebook Ads        | Daily campaign performance, Insights (any level and breakdown) | Core delivery metrics plus one explicit purchase action type; unified attribution requested. Insights: account, campaign, ad set or ad rows by day, week, month or whole period; age, gender, country, region, platform, placement and device breakdowns; reach, frequency, link and outbound clicks, video plays, leads and other actions; **Load columns** adds every conversion the account reports, custom conversions by name |
| LinkedIn Ads        | Daily campaign performance, Analytics (any level and audience) | Spend, delivery, conversions, leads, engagement and video by campaign group, campaign or creative, and by company, industry, job title, function, seniority, country, region, placement or device; daily, monthly or whole period |
| Microsoft Ads (Bing) | Daily campaign performance, 18 report levels (account, campaign, ad group, ad, keyword, search term, geography, user location, age and gender, professional demographics, audience, landing page, Shopping product, asset group, conversions by goal, goals and funnels, keyword impression share, website placements) | Every column Microsoft defines for the report through **Load columns**; daily, weekly, monthly, day-of-week or hour-of-day rows, or totals ranked by spend; **Find accounts** fills the account and customer IDs |
...
Reports default to 10,000 rows and support up to 30,000 within the provider and storage limits. Saved GA4 and GitHub reports can fetch across multiple executions: choose **Resume** when paused, or let the hourly scheduler continue, even for on-demand reports. Other sources and all previews fetch within one execution. The sheet is updated only when the complete report is ready. See [chunk continuation](docs/chunk-continuation.md) for limits and recovery. Numbers are written as numbers, IDs as text, and dates in ISO format. Snowflake exact decimals and integers wider than 15 digits remain text to preserve precision.

Saved reports and dashboards are private records in your Google account, scoped to the spreadsheet where you created them. Nothing about a report is stored in the spreadsheet except its output, so collaborators see the tables but not your report settings, SQL, connections or schedules, and a copied spreadsheet starts without saved reports. See [report storage](docs/report-storage.md).
...
Chat shows live actions while it works and formats answers with bold text, lists and tables. **Show completed actions (debug)** is on by default and keeps the action history below each answer; switch it off in Settings to hide successful histories. Errors and completed sheet updates remain visible. Use the **+** button for a new conversation. Under **Settings > AI provider**, edit **Maximum rows per chat report** (10,000 by default; 1 to 30,000) and general instructions. For account-specific rules, fill **Chat instructions** in the connection form; they save with the connection's **Save** button. All instructions share a 100,000-character limit; existing source rules remain available until you customize each connection. Ask, for example: "Create a new tab with the highest-spend campaign in each month, including spend, clicks and impressions." Chat aggregates first, ranks within each month and keeps currencies separate.
...
The sidebar has four tabs: **Reports** (the list, with the builder behind the + button), **Chat**, **Connections** (the list, with the editor behind +) and **Settings** (AI provider and credentials). Google Sheets sidebars are 300 px wide; the ⤡ button in the header opens the same UI in a larger window that leaves the sheet usable.
...
4. Choose **Extensions → DataMoov → Open**, grant the requested permissions, add a connection, and build your first report.
