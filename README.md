...
| ------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| Google Ads          | Daily campaign performance, YouTube video campaigns, Custom query (GAQL) | Spend, impressions, clicks, conversions, conversion value, optional segments; video views, view rate, CPV and quartiles; any resource (ad groups, ads, keywords, search terms, negative keywords) through one GAQL query |
...
2. Chat saves the plan and runs it: every dataset is fetched once and written to its own tab, headed by its source, connection, report, date range and refresh time. The Dashboard tab gets scorecards, native Sheets charts, a Data sources table and the numbers behind each chart. The answer links to every tab.
3. Open **Reports > Dashboards** to see the datasets, their tabs and rows, the last refresh and any error. **Refresh dashboard** fetches fresh data and rebuilds every tab and chart without AI. Charts are updated in place, so one you moved stays put.
...
A dashboard holds 1 to 6 datasets, up to 12 tiles and 20,000 rows in total, refreshed on demand in one atomic write: a failed source or blocked tab leaves everything as it was. Money in different currencies is split, never added. Plans stay private to their creator in this spreadsheet and do not travel with a copy; Remove deletes the setup and keeps the tabs. See [dashboard details and limits](docs/chat.md#dashboards).
