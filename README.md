...
- **Build refreshable dashboards in Chat:** Ask for a report combining multiple sources. Chat saves the setup, writes combined data and a summary to two tabs, and can add charts, formatting or native pivots. Refresh both tables later from the sidebar without another AI call. Live actions, source names and output tabs make each step visible.
...
Chat shows live actions while it works and formats answers with bold text, lists and tables. **Show completed actions (debug)** is on by default and keeps the action history below each answer; switch it off in Settings to hide successful histories. Errors and completed sheet updates remain visible. Use the **+** button for a new conversation. Under **Settings > AI provider**, edit **Maximum rows per chat report** (1 to 20,000) and save general or source-specific instructions, up to 100,000 characters combined. Ask, for example: "Create a new tab with the highest-spend campaign in each month, including spend, clicks and impressions." Chat aggregates first, ranks within each month and keeps currencies separate.
...
2. Chat saves the source queries and report rules, fetches every source, then writes the combined data and summary to two tabs. It can add native charts and formatting when requested.
3. Open **Reports > Dashboards** to see the sources, both output tabs, row counts, last refresh and any error. Click **Refresh dashboard** to fetch fresh data and rebuild both outputs without asking AI again. Live status explains which source or step is running.
